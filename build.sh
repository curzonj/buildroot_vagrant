...
yes 3 | env LC_ALL=C chroot /mnt bash -c 'apt-get update && apt-get -y install linux-generic'

env LC_ALL=C chroot /mnt bash <<"END_CHROOT"
set -e
set -x

apt-get -y install curl wget openssh-server sudo

echo vagrant-core > /etc/hostname
